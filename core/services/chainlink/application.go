...
func NewApplication(config *orm.Config, ethClient eth.Client, advisoryLocker postgres.AdvisoryLocker, keyStoreGenerator strpkg.KeyStoreGenerator, onConnectCallbacks ...func(Application)) Application {
	...
	if config.Dev() || config.FeatureOffchainReporting() {
		concretePW, err := offchainreporting.NewSingletonPeerWrapper(store.OCRKeyStore, store.Config, store.DB)
		if err == nil {
			...
			logger.Warnw("Off-chain reporting feature was enabled, but the libp2p peer failed to start. If you have any OCR jobs, they will not run correctly", "err", err)
			offchainreporting.RegisterJobType(store.ORM.DB, jobORM, store.Config, store.OCRKeyStore, jobSpawner, pipelineRunner, ethClient, logBroadcaster, nil)
		}
	}
	...
	app := &ChainlinkApplication{
		...
		// TODO: Clean up by putting more things in subservices instead of manually start/closing
